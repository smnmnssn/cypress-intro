describe("Kundlista, CRUD.", () => {
  ...
  it("ska kunna redigera kund", () => {
    cy.visit("/clients");
    cy.contains("Lisa Larsson").parent().find("#edit-button").click();
  ...
  it("ska kunna radera kund", () => {
    cy.visit("/clients");
    cy.get("#create-new-button").click();
    cy.get("input[name=name]").clear().type("Calle Klasson");
    cy.get("input[name=email]").clear().type("Calle@test.se");
    cy.get("input[name=address]").clear().type("Test Gatan 1");
    cy.get("button[type=submit]").click();
    cy.contains("Calle Klasson").parent().find("#delete-button").click();
